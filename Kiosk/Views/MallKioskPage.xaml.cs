...
namespace IntelligentKioskSample.Views
{
    ...
    /// </summary>
    [KioskExperience(Title = "Recomendação", ImagePath = "ms-appx:/Assets/mall.png", ExperienceType = ExperienceType.Kiosk)]
            ...
            if (this.currentRecommendation != null)
            {
                ...
                if (string.IsNullOrEmpty(alternativeUrl) && this.currentRecommendation.SpeechSentimentBehavior != null)
                {
                    ...
                    if (e.TextAnalysisSentiment <= 0.33)
                    {
                        ...
                        behaviorAction = this.currentRecommendation.SpeechSentimentBehavior.FirstOrDefault(behavior => string.Compare(behavior.Key, "Negativo", true) == 0);
                    }
                    else if (e.TextAnalysisSentiment >= 0.66)
                    {
                        // look for an override for positive sentiment
                        behaviorAction = this.currentRecommendation.SpeechSentimentBehavior.FirstOrDefault(behavior => string.Compare(behavior.Key, "Positivo", true) == 0);
                    }
